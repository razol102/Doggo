...
class OutdoorActivityRow extends StatelessWidget {
  ...
  const OutdoorActivityRow({super.key, required this.item, required this.dogId, required this.type});
  ...
  void _fetchItemDetails(BuildContext context, int itemId) async {
    try {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            title: Text(type == "activity" ? item["activity_type"].toString() : "${item["category"].toString().toUpperCase()} GOAL"),
            content: Column(
      ...
      print("Error fetching ${type == 'activity' ? 'activity' : 'goal'} details: $e");
  ...
  Widget build(BuildContext context) {
    ...
    final currentValue = double.tryParse(item["current_value"].toString()) ?? 0;
    final targetValue = double.tryParse(item["target_value"].toString()) ?? 1; // Avoid division by zero
