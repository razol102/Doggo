    ...
    try:
        ...
        with psycopg2.connect(**db) as connection:
            with connection.cursor() as cursor:
                ...
                calculate_calories(cursor, dog_id, new_dog_distance)
...
@fitness_routes.route('/api/dogs/fitness', methods=['GET'])
def get_dog_fitness():
    dog_id = request.args.get('dog_id')
    ...
    try:
        with psycopg2.connect(**db) as connection:
            with connection.cursor() as cursor:
                ...
                if not fitness_details:
                    raise ValueError("Fitness by dog and date does not exist.")
